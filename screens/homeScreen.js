...
const HomeScreen = () => {
  ...
  const fetchAddresses = async()=>{
    try {
      const response = await axios.get(`https://ecommercebackend-2xn3.onrender.com/addresses/${userId}`);
  ...
  return (
    <SafeAreaView style={{ flex: 1, background: 'white', paddingTop: Platform.OS === "android" ? 40 : 0 }}>
      <ScrollView>
        <View style={{ backgroundColor: '#00CED1', flexDirection: 'row', alignItems: 'center', padding: 10 }}>
          <Pressable style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'white', height: 33, flex: 1, marginHorizontal: 5, gap: 10, borderRadius: 5, paddingHorizontal: 10 }}>
       ...
       {/* <SliderBox
         ...
         <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {images.map((img, index)=>(
            <Image source={{uri: img}} key={index} style={{width: 400, height: 180, resizeMode: "cover", borderRadius: 20}}/>
          ))}
         </ScrollView>

        <View style={{ marginVertical: 10 }}>
          <Text style={{ fontSize: 18, fontWeight: 'bold', marginHorizontal: 10 }}>Trending Deals of the Week</Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', marginHorizontal: 5 }}>
            {deals.map((item, index) => (
              <Pressable key={index} style={{ width: '45%', marginVertical: 10 }} 
              onPress={()=>navigation.navigate('productInfo', {
                id: item.id, title: item.title, price: item?.price, carouselImages: item.carouselImages, color: item?.color, size: item?.size, oldPrice: item?.oldPrice, item: item
              })}>
                <Image source={{ uri: item.image }} style={{ width: '100%', height: 180, resizeMode: 'contain' }} />
        ...
        <View style={{ marginTop: 20 }}>
            <Text style={{ fontSize: 18, fontWeight: 'bold', marginHorizontal: 10 }}>Today's Deals</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {offers.map((item, index) => (
                    <Pressable key={index} style={{ marginHorizontal: 10, alignItems: 'center' }}>
                        <Image source={{ uri: item.image }} style={{ width: 150, height: 150, resizeMode: 'contain' }} />
                        <Pressable style={{ backgroundColor: '#be0a0e', paddingVertical: 5, paddingHorizontal: 10, borderRadius: 5, marginTop: 10 }}>
                            <Text style={{ color: 'white', fontWeight: 'bold' }}>Upto {item.offer} off</Text>
                        </Pressable>
                    </Pressable>
                ))}
            </ScrollView>
